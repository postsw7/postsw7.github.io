...
import { trackVisit, trackCommand, trackUnknownCommand, trackDemoError } from '../core/analytics'
...
interface CommandItem extends BaseItem {
  type: 'command'
  prompt: string
  content: string
  meta: CommandMeta
}
interface OutputItem extends BaseItem {
  type: 'output'
...
export function CLI(): JSX.Element {
  ...
  useEffect(() => {
    ...
    try {
      ;(globalThis as any).__CLI_CWD__ = cwd
    } catch (e) {
      /* no-op */
    }
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      setOutput((prev) =>
        prev.map((line) =>
          line.id === jgrepSpinner.id
            ? {
                ...line,
                type: 'info',
                content: (
                  <span className="text-gray-400">{frameChar} loading jgrep runtime...</span>
                ),
              }
            : line,
        ),
  ...
  function finalizeCommand(_commandId: string | number, status: Status) {
  ...
  function handleKeyDown(e: KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === 'Enter') {
      ...
      if (
        suppressedAutocompleteEnterRef.current &&
        suppressedAutocompleteEnterRef.current === input
      ) {
      ...
      addOutput({
        ...
        content: (
          <div className="flex flex-wrap gap-4">
            {entries.map((e) => (
              <span key={e.name} className={e.type === 'dir' ? 'text-accent' : ''}>
    ...
    if (cmdName === 'jgrep') {
      ...
      const buildNodeFromResult = (result: any) => {
        if (result.format === 'lines') {
          ...
          const widths = result.header.map((h: string, i: number) =>
            Math.max(h.length, ...result.rows.map((r: string[]) => (r[i] || '').length)),
          )
          const pad = (v: string, i: number) => v + ' '.repeat(widths[i] - v.length)
          const linesTxt = [
            result.header.map(pad).join('  '),
            ...result.rows.map((r: string[]) => r.map(pad).join('  ')),
          ]
      ...
      const spinnerId = addOutput({
        type: 'info',
        content: <span className="text-gray-400">⠋ loading jgrep runtime...</span>,
      })
      ...
      try {
        ...
        const replace = () => {
          setOutput((prev) =>
            prev.map((line) =>
              line.id === spinnerId
                ? { ...line, type: 'output', content: buildNodeFromResult(result) }
                : line,
            ),
          )
            ...
            prev.map((line) =>
              line.id === spinnerId
                ? {
                    ...line,
                    type: 'error',
                    content: err?.message || 'jgrep: error: unknown failure',
                  }
                : line,
            ),
          )
    ...
    <div className="min-h-screen bg-app text-fg p-4 sm:p-8 font-mono">
      <div className="max-w-5xl mx-auto">
        <div className="border border-gray-600 rounded-lg overflow-hidden shadow-2xl">
          <div className="bg-header px-4 py-2 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-4 text-sm text-gray-400">
                {STRINGS.promptBase} ~ {STRINGS.versionLabel}
              </span>
            </div>
            <a
              href="/v1/"
              className="text-xs text-gray-400 hover:text-accent transition-colors"
              title="View legacy portfolio"
            >
              v1 →
            </a>
          </div>
          <div
            ref={scrollRef}
            className="bg-terminal p-4 sm:p-6 min-h-[600px] max-h-[80vh] overflow-y-auto"
                    ...
                    <span
                      ...
                      className={
                        idx === suggestions.index
                          ? 'px-1 rounded bg-[#00ffa6] text-black'
                          : 'text-[#7ee787]'
                      }
                    >
                      {c}
        ...
        <footer className="mt-4 text-center text-sm text-gray-500">
          <a
            href="https://github.com/postsw7/postsw7.github.io"
            target="_blank"
